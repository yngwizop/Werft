...
class NetBoxService:
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        if not self.settings.netbox_url or not self.settings.netbox_token:
            raise NetBoxError("NETBOX_URL and NETBOX_TOKEN must be configured")
    ...
    def reserve_ip(self, subnet_cidr: str, ticket_id: str, hostname: str) -> ReservedIp:
        """Find prefix by CIDR and allocate the next available IP."""
        prefixes = list(self.nb.ipam.prefixes.filter(prefix=subnet_cidr))
        if not prefixes:
            # Try contains match if exact filter fails depending on NetBox version.
            prefixes = [
                p
        ...
        if not prefixes:
            raise NetBoxError(f"Prefix not found in NetBox: {subnet_cidr}")

        prefix = prefixes[0]
        available = prefix.available_ips.list()
        if not available:
            raise NetBoxError(f"No free IPs in prefix {subnet_cidr}")

        # available_ips.list() may return strings or objects depending on version.
        first = available[0]
        ...
        return ReservedIp(address=address_only, cidr=str(created.address), ip_id=int(created.id))
    ...
    def create_vm_if_possible(
        ...
        *,
        ...
        """Create a NetBox virtual machine when virtualization data exists; else return None."""
        try:
            payload: dict = {
                "name": hostname,
                "status": "planned",
            ...
            if cluster_name:
                ...
                # Without cluster NetBox usually rejects VM create.
    ...
    def finalize(self, *, ip_id: int, vm_id: int | None = None) -> None:
    ...
    def release_ip(self, ip_id: int | None) -> None:
        if not ip_id:
            return
        try:
            ip = self.nb.ipam.ip_addresses.get(ip_id)
    ...
    def delete_vm(self, vm_id: int | None) -> None:
        if not vm_id:
            return
        try:
            vm = self.nb.virtualization.virtual_machines.get(vm_id)
    ...
    def compensate(self, *, ip_id: int | None = None, vm_id: int | None = None) -> None:
